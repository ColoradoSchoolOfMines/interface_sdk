...
public class HardwareManager {
	...
	/*
	 * Map<String, List<String>> deviceSupports = metaData.getDeviceSupports();
	 * Map<String, String> driverPaths = metaData.getDevices(); Set<String>
	 * deviceKeys = deviceSupports.keySet();
	 * 
	 * // Look for all REQUIRED functionalities. // Find drivers that support
	 * that functionality. // Instantiate and store into cache.
	 * 
	 * 
	 * Set<String> moduleFuncs = currentModuleInputTypes.keySet(); List<String>
	 * requiredFuncs = new ArrayList<String>(); for (String moduleFunc :
	 * moduleFuncs) { if (currentModuleInputTypes.get(moduleFunc) ==
	 * DependencyType.REQUIRED) { for (String device : deviceKeys) { if
	 * (deviceSupports.get(device).contains(moduleFunc)) { // This device
	 * supports it! Instantiate and store // into the cache
	 * 
	 * //addDeviceToCache(device) String driverPath = driverPaths.get(device);
	 * try { Class<? extends DriverInterface> cl =
	 * Class.forName(driverPath).asSubclass(DriverInterface.class); Constructor
	 * ctor = cl.getConstructor(); DriverInterface iDriver = (DriverInterface)
	 * ctor.newInstance();
	 * 
	...
	/*
	 * Map<String, String> supportedDevices = metaData.getDevices(); Map<String,
	 * List<String>> deviceFuncs = metaData.getDeviceSupports(); Set<String>
	 * keys = supportedDevices.keySet();
	 * 
	 * 
	 * for (String device : keys) { // Grab all the devices from the meta-data
	 * String driver = supportedDevices.get(device); try {
	 * 
	 * Class<? extends DriverInterface> cl =
	 * Class.forName(driver).asSubclass(DriverInterface.class); Constructor ctor
	 * = cl.getConstructor(); DriverInterface iDriver = (DriverInterface)
	 * ctor.newInstance(); // Check whether the device is available if
	 * (iDriver.isAvailable()) { // Cache the driver
	 * deviceDriverCache.put(driver, iDriver);
	 * 
	 * // Go through each functionality for this driver // Add it to our
	 * 'devices' storage unit // This is because we assume if a device is
	 * available, // then all the functionalities it supports are also //
	 * available.
	 * 
	 * List<String> funcs = deviceFuncs.get(device); for (String func : funcs) {
	 * if (devices.containsKey(func)) { devices.get(func).add(driver); } else {
	 * List<String> availFuncs = new ArrayList<String>();
	 * availFuncs.add(driver); devices.put(func, availFuncs); } }
	 * 
	 * } } catch (ClassNotFoundException e) {
	 ...
	 * InvalidConfigurationFileException(e.getMessage()); } catch
